 ...
 */
void MinMin::ScheduleAvailTasks(std::list<std::shared_ptr<Activation>> avail_tasks, Solution &solution) {
    while (!avail_tasks.empty()) {
        ...
        for (const auto &task: avail_tasks) {
            ...
            for (const auto &vm: virtual_machines_) {
                ...
                double objective_value = new_solution.ScheduleActivation(task, vm);
        ...
        std::shared_ptr<Activation> my_task = activations_[iteration_minimal_task_id];
...
void MinMin::Run() {
    ...
    for (const auto &task: activations_) {
